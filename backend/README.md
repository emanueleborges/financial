...
- **Grafana**: http://localhost:3001 (user/senha: `admin` / `admin`)
...
| GET | `/api/v1/users/{document}/transactions/export` | Extrato PDF |
...
```bash
cd backend
mvn test          # unitários + integração (Testcontainers)
mvn verify        # inclui relatório Jacoco
```

- Unitários: JUnit 5 + Mockito (domínio e use cases)
- Integração (`src/test/.../integration/`): PostgreSQL + Kafka via Testcontainers, profile `it` (Flyway real, cache in-memory). Requer Docker Desktop rodando; sem Docker os ITs são **skipped** (`disabledWithoutDocker`)
- Jacoco para cobertura (meta do desafio: > 70%)

```bash
# ITs com Docker ativo
mvn test -Dtest='*IntegrationTest'
```

> **Nota:** Docker Engine 29+ exige API ≥ 1.44 — o projeto inclui `src/test/resources/docker-java.properties`.
